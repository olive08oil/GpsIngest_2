...
- このリポジトリは `プロジェクトルール.md` に従って運用する。
- ルート `docs/obsidian/` は全体統括メモとして維持する。
- ソリューションに含まれる各 `.csproj` フォルダにも `docs/obsidian/` を置き、プロジェクト単位の状態を記録する。
- Codexを主担当AIとし、Qwen / Continue / Roo Code / Cline などの補助AI作業は `agent_review_queue.md` でレビュー待ち管理する。
- Git操作はソリューションルートで実行する。
